...
class ProjectDashboardCartera(models.Model):
    ...
    total_general = fields.Float(string='Total General', compute='_compute_totals', store=True)
    total_cotizando_fabricar = fields.Float(string='Por Fabricar', compute='_compute_totals', store=True)
    total_en_proceso = fields.Float(string='Fabricando', compute='_compute_totals', store=True)
    total_terminado = fields.Float(string='Terminado', compute='_compute_totals', store=True)
    ...
    def _compute_totals(self):
        ...
        for record in self:
            ...
            for proj in all_projects:
                ...
                if stage in ('Fabricando',  'Instalando', 'Instalando-Fabricando', 'Entregando'):
