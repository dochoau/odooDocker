# wizards/project_contract_upload_wizard.py
import os
import base64
...
class ProjectcontractUploadWizard(models.TransientModel):
    _name = 'project.contract.upload.wizard'
    _description = 'Cargar contrato del Proyecto'
    ...
    def action_upload_contract(self):
        ...
        self.filename_arch ="contrato.pdf"
        if not self.filename_arch.lower().endswith('.pdf'):
            raise UserError("El archivo debe ser un PDF.")

        project_folder = os.path.join(UPLOAD_DIR, self.project_id.name or '', 'contrato')
        ...
        self.project_id.contrato_loaded = True
